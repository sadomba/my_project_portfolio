...
if(isset($_POST['back']))
 ...
 $sql="UPDATE `complete` SET `complete` = 1 WHERE `course` = '$course' ";
 $query=$dbh->prepare($sql);
  $query->execute();

 
echo '<script>alert("all editions are being locked.")</script>';
                                                    ...
                                                    <div class="col-md-8 col-lg-10 col-xl-5 col-xxl-4" style="width: 700.5px;">
                                                        <div class="card mb-5">
                                                            <div class="card-body p-sm-5" style="background:blue;width: 600.5px;">
                                                                <h2 class="text-center mb-4 text-warning">Export to API</h2>
                                                                <form method="post">
                                                                <div class="mb-3"><input class="form-control text-black form-control-user" required type="text"  name="url" placeholder="enter API url"></div>
                                                                    <div class="mb-3 text-light"> <h2><?php echo $course?></h2> </div>

                                    <div class="mb-3"><button class="btn btn-warning" type="submit" name="choose">export</button></div>                     
                                                                </form><hr>  <form method="post"> 
                                          <div class="mb-3"><button class="btn btn-warning" type="submit" name="back">back</button></div>                                    
                                                                </form>
                                                            </div>
                                                        </div>
                                                    </div>
